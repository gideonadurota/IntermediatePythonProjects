...
def get_currency(currency: str, rates: dict) -> float:
    ...
    if currency in rates.keys():
        ...
        raise ValueError(f'Currency {currency} is not a valid currency')
...
def main():
    ...
    amt = input('Enter amount to convert: ')
    base = input('Enter base currency code: ')
    vs = input('Enter target currency code: ')

    convert_currency(float(amt), base=base, vs=vs, rates=rates)
